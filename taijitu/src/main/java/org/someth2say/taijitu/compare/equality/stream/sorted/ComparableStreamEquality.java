 ...
 */
public class ComparableStreamEquality<T> extends AbstractStreamEquality<T, ComparableEquality<T>> implements org.someth2say.taijitu.compare.equality.stream.StreamEquality<T> {

    private static final Logger logger = LoggerFactory.getLogger(ComparableStreamEquality.class);

    public ComparableStreamEquality(Equality<T> equality, ComparableEquality<T> categorizer) {
    ...
    public static <T> List<Mismatch<?>> compare(Stream<T> source, Stream<T> target, ComparableEquality<T> comparer, Equality<T> equality) {
    ...
    private static <T> int flushMissings(ComparableEquality<T> comparer, List<Mismatch<?>> newresult,
