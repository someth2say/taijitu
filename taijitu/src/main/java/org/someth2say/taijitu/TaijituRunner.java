...
class TaijituRunner implements Callable<List<Mismatch>> {
	...
	private <T> List<Mismatch> runComparison(IComparisonCfg iComparisonCfg) {
		...
		List<FieldDescription<?>> commonFields = getCommonFields(
				sourceDatas.stream().map(sd -> sd.mappedSource).collect(Collectors.toList()));

		// A. Get fields for each comparison:
		// A.1.- Identity
		List<FieldDescription<?>> identityFields = commonFields.stream()
				.filter(fd -> iComparisonCfg.getKeyFields().contains(fd.getName())).collect(Collectors.toList());
		// A.2.- CategoryEquality (for mapping) and ComparableEquality (for sorted)
		// TODO: Currently we have no difference between category and sorting fields. So
		// we just use all non-key fields
		List<FieldDescription<?>> nonIdentityFields = commonFields.stream().filter(fd -> !identityFields.contains(fd))
				.collect(Collectors.toList());

		// 3. Obtain ValueExtractors and Equalities
		sourceDatas.forEach((SourceData<?, T> sd) -> {
			sd.categoryEaEs = identityFields.stream()
					.map(fd -> buildExtractorAndComparableEquality(iComparisonCfg, sd, fd))
					.collect(Collectors.toList());
			logger.debug("Category EaEs for {}:\n {}", sd.source.getName(),
					IntStream
							.range(0, identityFields.size()).mapToObj(fdIdx -> sd.categoryEaEs.get(fdIdx).toString()
									+ "(" + identityFields.get(fdIdx).toString() + ")")
							.collect(Collectors.joining(",\n ")));
			sd.equalityEaEs = nonIdentityFields.stream().map(fd -> buildExtractorAndEquality(iComparisonCfg, sd, fd))
					.collect(Collectors.toList());
			logger.debug("Equality EaEs for {}:\n {}", sd.source.getName(),
					IntStream.range(0, nonIdentityFields.size())
							.mapToObj(fdIdx -> sd.equalityEaEs.get(fdIdx).toString() + "("
									+ nonIdentityFields.get(fdIdx).toString() + ")")
							.collect(Collectors.joining(",\n ")));
		});

		// 4. Build CompositeEqualities with ValueExtractors and ValueEqualities
		// TODO: This assumes all sources use the same extractors! Else, we need an
		// "HybridCompositeEquality", providing, for each field on a) extractors for
		// each source, and b) valueEqualities
		...
		final StreamEquality<T> streamEquality = StreamEqualityRegistry.getInstance(strategyName, equality,
				categorizer);
	...
	private <T, V> ExtractorAndEquality<T, V> buildExtractorAndEquality(IComparisonCfg iComparisonCfg,
			SourceData<?, T> sd, FieldDescription<V> fd) {
	...
	private <T, V> ExtractorAndComparableCategorizerEquality<T, V> buildExtractorAndComparableEquality(
			IComparisonCfg iComparisonCfg, SourceData<?, T> sd, FieldDescription<V> fd) {
		Function<T, V> extractor = sd.mappedSource.getExtractor(fd);
		if (extractor == null) {
			throw new RuntimeException("Can't obtain extractor for field " + fd);
		}
	...
	private <V> Equality<V> getEquality(FieldDescription<V> fd, IComparisonCfg iComparisonCfg) {
		IEqualityCfg iEqualityCfg = getEqualityConfigFor(fd.getClazz(), fd.getName(),
				iComparisonCfg.getEqualityConfigs());
		return ValueEqualityRegistry.getInstance(iEqualityCfg.getName(), iEqualityCfg.getEqualityParameters());
	}

	private <V> ComparableCategorizerEquality<V> getComparableEquality(FieldDescription<V> fd,
			IComparisonCfg iComparisonCfg) {
		List<IEqualityCfg> iEqualityCfgs = getEqualityConfigsFor(fd.getClazz(), fd.getName(),
				iComparisonCfg.getEqualityConfigs());
		Optional<ComparableCategorizerEquality<V>> first = iEqualityCfgs.stream()
				.map(cfg -> ValueEqualityRegistry.getInstance(cfg.getName(), cfg.getEqualityParameters()))
				.filter(eq -> eq instanceof ComparableCategorizerEquality)
				.map(eq -> (ComparableCategorizerEquality<V>) eq).findFirst();
		if (first.isPresent()) {
			return first.get();
		}
	...
	private List<IEqualityCfg> getEqualityConfigsFor(final String fieldClass, final String fieldName,
			final List<IEqualityCfg> equalityCfgs) {
	...
	private IEqualityCfg getEqualityConfigFor(final String fieldClass, final String fieldName,
			final List<IEqualityCfg> equalityConfigIfaces) {
